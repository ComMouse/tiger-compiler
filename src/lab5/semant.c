...
static struct expty transExp(Tr_level level, S_table venv, S_table tenv, A_exp a, Temp_label breaklbl) {
  switch (a->kind) {
    ...
    case A_callExp: {
      ...
      return expTy(Tr_callExp(func->u.fun.label, explist), func->u.fun.result);
    }
    case A_opExp: {
    ...
    case A_forExp: {
      ...
      S_endScope(venv);
      return expTy(Tr_forExp(explo.exp, exphi.exp, body.exp, forbreak), Ty_Void());
    }
    case A_breakExp: {
    ...
    case A_letExp: {
      ...
      for (d = a->u.let.decs; d; d = d->tail)
        transDec(level, venv, tenv, d->head, breaklbl);
...
static void transDec(Tr_level level, S_table venv, S_table tenv, A_dec d, Temp_label breaklbl) {
  switch (d->kind) {
    ...
    case A_varDec: {
      ...
      S_enter(venv, d->u.var.var, E_VarEntry(Tr_allocLocal(level, TRUE), t));
      break;
    }
    case A_typeDec: {
      A_nametyList l;
